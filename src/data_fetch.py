...
def download_all(
    ...
    # Fallback: load existing raw CSVs if download keeps failing
...
def clean_single(df: pd.DataFrame) -> pd.DataFrame:
    """Standardize one ticker's DataFrame: datetime index, lowercase columns, no NaNs in Close."""
    out = df.copy()
...
def build_close_wide(frames: dict[str, pd.DataFrame] | None = None) -> pd.DataFrame:
    ...
    Uses sequential merge on datetime index.
    """
...
if __name__ == "__main__":
    ...
    prices = run_pipeline(use_cache=not args.no_cache, demo=args.demo)
    print("Saved wide price table:", prices.shape)
    print(prices.head())
