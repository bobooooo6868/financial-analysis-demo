...
def vectorized_summary(returns: pd.DataFrame) -> pd.DataFrame:
    """
    Same statistics as summary_stats, demonstrating NumPy broadcasting
    on a 2D array without Python loops over tickers.
    """
    arr = returns.to_numpy(dtype=float)
    # Broadcast-friendly: stats along axis=0 (time) for each column
